...
const technologyCategories = [
  {
    ...
    technologies: [
      ...
      {
        name: "MUI",
        description: "Material UI knihovna komponent pro React",
        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/materialui/materialui-original.svg",
        color: "#007FFF",
      },
      {
        name: "Blazor",
        description: "Webový framework pro tvorbu interaktivního UI v C#",
  ...
  },
  {
    ...
    title: "Backend Vývoj",
    description:
      "Technologie pro vývoj serverových aplikací a databázové systémy",
    technologies: [
      {
        name: "PHP",
    ...
    technologies: [
      {
        name: "C#",
        description:
          "OOP jazyk pro vývoj desktopových, webových a mobilních aplikací",
  ...
  },
  {
    ...
    technologies: [
      ...
      {
        name: "Visual Studio",
        description: "Integrované vývojové prostředí pro .NET, C# a C++",
      ...
      {
        name: "IntelliJ IDEA",
        description: "Integrované vývojové prostředí pro Java & Kotlin",
  ...
  },
  {
    ...
    title: "Embedded",
    description:
      "Hardwarové platformy, mikrokontroléry a vývoj embedded systémů",
    technologies: [
      {
        name: "STM32",
        description:
          "32bitové mikrokontroléry od STMicroelectronics pro embedded projekty",
        icon: "https://cdn.jsdelivr.net/gh/simple-icons/simple-icons/icons/stmicroelectronics.svg",
        color: "#03234B",
      },
      {
        name: "C",
        description: "Programovací jazyk pro nízkoúrovňový a embedded vývoj",
...
const TechnologyPage = () => {
  return (
    <>
      <SEO
        title="Technologie - ECL IT | Nástroje pro moderní vývoj"
        description="Technologie a nástroje, které používám pro tvorbu webových aplikací - React, C#, Blazor, PHP, Node.js, databáze, embedded systémy a další moderní technologie."
        canonicalUrl="https://www.ecl-it.cz/technologie"
        keywords="React, C#, .NET, Blazor, MUI, Node.js, PHP, HTML, CSS, JavaScript, TypeScript, MySQL, PostgreSQL, Embedded, STM32, C, Arduino, Visual Studio, webové technologie, frontend, backend"
      />
      <Navigation />
      ...
      <div className="pt-24 min-h-screen bg-gray-50">
        {" "}
        {/* pt-24 = padding-top pro offset od navigace, min-h-screen = minimální výška celé obrazovky, bg-gray-50 = světle šedé pozadí */}
        {/* Kontejner pro nadpis */}
        <div className="container mx-auto px-4 py-8">
          {" "}
          {/* container mx-auto = centrovaný kontejner, px-4 = horizontální padding, py-8 = vertikální padding */}
          {/* Hlavní nadpis */}
          <motion.h1
          ...
          >
            Nástroje a technologie, které používáme pro tvorbu webových
            aplikací, softwarových projektů a hardware řešení.
          </motion.p>
          {/* Kategorie technologií */}
          <div className="space-y-16">
            {" "}
            {/* space-y-16 = vertikální mezery mezi sekcemi */}
            {technologyCategories.map(
              (
                category,
                categoryIndex, // Mapování přes kategorie
              ) => (
                <motion.section
                  key={category.id}
                ...
                >
                  ...
                  <motion.div className="text-center mb-8">
                    {" "}
                    {/* text-center = centrovaný text, mb-8 = spodní margin */}
                    <h2
                    ...
                    <p className="text-gray-600 text-sm md:text-base">
                      {" "}
                      {/* text-gray-600 = středně šedá, text-sm md:text-base = responzivní velikost */}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
                    {" "}
                    {/* responzivní grid - 2 sloupce na mobilu, až 6 na velkých obrazovkách, gap-6 = mezery */}
                    {category.technologies.map(
                      (
                        tech,
                        techIndex, // Mapování přes technologie v kategorii
                      ) => (
                        <motion.div
                          key={tech.name}
                        ...
                        >
                          ...
                          <div className="flex justify-center mb-4">
                            {" "}
                            {/* flex justify-center = centrování, mb-4 = spodní margin */}
                            <ImageWithLoader
                              src={tech.icon}
                              alt={tech.name}
                          ...
                          {/* Název technologie */}
                          <h3 className="text-center font-semibold text-gray-800 mb-2 group-hover:text-cyan-600 transition-colors duration-300">
                            {" "}
                            {/* text-center = centrovaný text, font-semibold = půl-tučné, text-gray-800 = tmavě šedá, mb-2 = spodní margin, group-hover:text-cyan-600 = změna barvy při hoveru, transition-colors = plynulá změna barev */}
                            {tech.name}
                          </h3>

                          {/* Hover overlay s popisem */}
                          <div className="absolute inset-0 bg-white/95 backdrop-blur-sm rounded-xl p-4 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-center">
                            {" "}
                            {/* absolutní overlay, bg-white/95 = poloprůhledné bílé pozadí, backdrop-blur-sm = rozmazání pozadí, rounded-xl = zaoblené rohy, p-4 = padding, opacity-0/100 = ovládání průhlednosti, group-hover = zobrazení při hoveru, flex flex-col justify-center = vertikální centrování */}
                            <h4
                            ...
                            <p className="text-sm text-gray-600 text-center leading-relaxed">
                              {" "}
                              {/* text-sm = malý text, text-gray-600 = středně šedá, text-center = centrovaný, leading-relaxed = větší řádkování */}
                              {tech.description}
                            </p>
                          </div>
                        </motion.div>
